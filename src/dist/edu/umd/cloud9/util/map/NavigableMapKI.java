...
/**
 * A {@link SortedMapKI} extended with navigation methods returning the closest matches for given
 ...
 *
 * <p>
 * A {@code NavigableMapKI} may be accessed and traversed in either ascending or descending key order.
 ...
 * NavigableMapKI} must implement the {@code NavigableMapKI} interface.
 *
 * <p>
 ...
 *
 * <p>
 * Implementations of entry-returning methods are expected to return {@code MapKI.Entry} pairs
 ...
 *
 */
public interface NavigableMapKI<K> extends SortedMapKI<K> {
  /**
   ...
   */
  MapKI.Entry<K> lowerEntry(K key);
   ...
   * @throws NullPointerException if the specified key is null and this map does not permit null keys
   */
  MapKI.Entry<K> floorEntry(K key);
   ...
   * @throws NullPointerException if the specified key is null and this map does not permit null keys
   */
  MapKI.Entry<K> ceilingEntry(K key);
   ...
   * @throws NullPointerException if the specified key is null and this map does not permit null keys
   */
  MapKI.Entry<K> higherEntry(K key);
   ...
   */
  NavigableMapKI<K> descendingMap();
   ...
   *           if {@code fromKey} is greater than {@code toKey}; or if this map itself has a restricted
   *           range, and {@code fromKey} or {@code toKey} lies outside the bounds of the range
   */
  NavigableMapKI<K> subMap(K fromKey, boolean fromInclusive, K toKey, boolean toInclusive);
   ...
   *           of the range
   */
  NavigableMapKI<K> headMap(K toKey, boolean inclusive);
   ...
   *           bounds of the range
   */
  NavigableMapKI<K> tailMap(K fromKey, boolean inclusive);

  // Inherited
  SortedMapKI<K> subMap(K fromKey, K toKey);
  SortedMapKI<K> headMap(K toKey);
  SortedMapKI<K> tailMap(K fromKey);
}
